...
const svg=`<svg id="journeyArt" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="4800" height="900" viewBox="0 0 4800 900" role="group" aria-label="循光而行，四段可探索的旅行长卷" style="font-family:Arial,'Microsoft YaHei','PingFang SC',sans-serif;color:#f2f1ed;fill:#f2f1ed">
...
<defs>
 ...
 <mask id="cl_nightMask" maskUnits="userSpaceOnUse" x="3050" y="0" width="1750" height="900"><rect x="3050" width="1750" height="900" fill="url(#cl_nightFade)"/></mask>
 ...
 <clipPath id="cl_galleryClip"><rect x="2420" y="405" width="710" height="315" rx="3"/></clipPath>
...
<rect width="4800" height="900" fill="#07172c"/>
<image data-asset="panorama" href="${imageAssets.panorama}" x="0" y="0" width="3650" height="900" preserveAspectRatio="xMidYMid slice"/>
<image data-asset="night" href="${imageAssets.night}" x="3050" y="0" width="1750" height="900" preserveAspectRatio="xMidYMid slice" mask="url(#cl_nightMask)"/>
<rect width="4800" height="900" fill="url(#cl_shade)"/>
...
${heading('canyonTitle1','canyonTitle2',2395,232,62)}
${text('canyonBody',2400,365,25,'fill="#d7e0e8"')}
${['mountain','water','sky'].map((key,i)=>`<g id="cl_scene_${key}" visibility="${i===0?'visible':'hidden'}">${select(key)}<g clip-path="url(#cl_galleryClip)"><svg x="2420" y="405" width="710" height="315" viewBox="${key==='mountain'?'1240 150 932 420':key==='water'?'1550 380 570 250':'0 0 1536 680'}" preserveAspectRatio="xMidYMid slice"><image data-asset="${key}" href="${imageAssets[key]}" width="${key==='sky'?1536:2172}" height="${key==='sky'?1024:724}" preserveAspectRatio="xMidYMid slice"/></svg><rect x="2420" y="654" width="710" height="66" fill="#03111f" fill-opacity=".83"/>${text(key+'Title',2445,697,25)}</g></g>`).join('')}
<rect x="2420" y="405" width="710" height="315" rx="3" fill="none" stroke="#c2cedd" stroke-opacity=".45"/>
${['mountain','water','sky'].map((key,i)=>`<g id="cl_view_${key}" class="cl-hit" role="button" tabindex="0" aria-label="${esc(config[key+'Label'])}" transform="translate(${2420+i*238} 737)"><rect class="cl-hover" width="224" height="69" rx="2" fill="#091b2f" fill-opacity=".6" stroke="#9daebd" stroke-opacity=".35"/><g visibility="${i===0?'visible':'hidden'}">${select(key)}<rect width="224" height="69" rx="2" fill="#cfae77" fill-opacity=".22" stroke="#e4c996"/><path d="M77 60h70" stroke="#e4c996" stroke-width="2"/></g>${text(key+'Label',112,43,25,'text-anchor="middle"')}</g>`).join('')}
...
<g id="cl_constellation" visibility="hidden"><set attributeName="visibility" to="visible" begin="cl_star_on.click" end="cl_star_off.click"/>
