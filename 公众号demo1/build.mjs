...
${['mountain','water','sky'].map((key,i)=>`<g id="cl_view_${key}" class="cl-hit" role="button" tabindex="0" aria-label="${esc(config[key+'Label'])}" aria-pressed="${i===0}" transform="translate(2370 ${300+i*160})"><rect width="150" height="132" fill="transparent"/><circle cx="75" cy="48" r="38" fill="#091b2f" fill-opacity=".7" stroke="#9daebd" stroke-opacity=".5"/><circle class="avatar-orbit" cx="75" cy="48" r="43" pathLength="100" style="animation-delay:-${i}s"/><text x="75" y="59" text-anchor="middle" font-size="28" fill="#e4c996">0${i+1}</text><g data-tab-active="${key}" visibility="${i===0?'visible':'hidden'}">${select(key)}<circle cx="75" cy="48" r="43" fill="none" stroke="#e4c996" stroke-width="1"/><circle class="avatar-halo" cx="75" cy="48" r="49"/><circle class="avatar-orbit avatar-orbit-gold" cx="75" cy="48" r="43" pathLength="100"/></g>${text(key+'Label',75,115,27,'text-anchor="middle"')}</g>`).join('')}
