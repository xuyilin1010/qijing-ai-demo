(() => {
  ...
  function warmNearby(){
    ...
    const chapterWidth=rotated?innerHeight:innerWidth;
    const chapter=Math.max(0,Math.floor(position()/chapterWidth));
    startImage(landscapeImages[Math.min(chapter,landscapeImages.length-1)]);
    startImage(landscapeImages[Math.min(chapter+1,landscapeImages.length-1)]);
    if(chapter>=1)startImages($('.product-reveal'));
    if(chapter>=2)startImages($('.product-gif-slot'));
    if(chapter>=3)startImages($('.feature-panel.active'));
    if(chapter>=5)startImage($('.story-panel[data-story="0"] img'));
  }
  function progress(){const fraction=maximum()>0?position()/maximum():0;$('#progress').style.transform=`${rotated?'scaleY':'scaleX'}(${Math.max(.008,fraction)})`;if(!experience.hidden){const chapterWidth=rotated?innerHeight:innerWidth;if(position()>.48*chapterWidth)$('.product-reveal')?.classList.add('is-visible');warmNearby();}schedulePaint();}
